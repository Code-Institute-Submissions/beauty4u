...
def booking (request):
    ...
    """ 
    A view that will render the main booking app for the user to select services 

    """

    username = request.user.username
    name = get_object_or_404(User, username=username)
    #Get profile of user logged in (They must have a phone number stored before making a booking to ensure they can be contacted)
    ...
    # If it is their first time booking, ask them for their name and phone number
...
def select_staff(request):
    ...
    """ 
    A view that will render the select staff template for the user to select a staff member for their booking
...
def select_time(request):
    ...
    """ 
    A view that will render the select date and time template for the user to select a date and time for their booking
    Takes posted data from booking.js
...
def confirm_booking(request):
    ...
    """ 
    A view that will render the confirm booking template to summarise the boking and ask the user to confirm 
    Takes posted data from booking.js
...
def booking_success(request):
    ...
    """ 
    A view that will process the booking, store it in the database and send emails to the salon owner and customer 
    Takes posted data from booking.js
    """
    ...
    if request.method == "POST":
        ...
        booking = Bookings.objects.create(username=username, service=services, date=date, time=time, staff=staff, customer_name=name)
        booking.save()

        #Email        
        cust_email = request.user.email
        subject = render_to_string('booking/booking_email_subject.txt')
        body = render_to_string('booking/booking_email_body.txt', {'date': date, 'time': time, 'name': name, 'staff': staff, 'services': services, 'cost': cost, 'username': username})

        #Email to Staff
        subject2 = render_to_string('booking/booking_email_subject2.txt')
        body2 = render_to_string('booking/booking_email_body2.txt', {'date': date, 'time': time, 'name': name, 'staff': staff, 'services': services, 'cost': cost, 'username': username, 'email': cust_email})
