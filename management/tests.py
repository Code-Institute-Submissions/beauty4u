...
class testBookingModel(TestCase):
    ...
    def test_confirm_booking(self):
        ...
        A test that checks when a booking id is posted to update_booking_status, that the confirmed status of the booking is changed and the correct booking id is returned 

        """

        request = self.factory.post(reverse('update_booking_status'), {'customer_name': 'David', 'confirmBooking': True, 'bookingDate': '01/06/2021', 'time': '09:00', 'bookingId': self.booking_id })
        request.user = self.user
        response = update_booking_status(request)
        self.assertEqual(json.loads(response.content)['booking_id'], self.booking_id)
        self.assertEqual(json.loads(response.content)['confirmedStatus'], 'True')

    def test_shipping_setting(self):

        """

        A test that checks when the toggle click & collect option is toggled to no, that the setting is set to no 

        """

        request = self.factory.post(reverse('save_data'), {'settingName': 'Click & Collect', 'settingStatus': 'True'})
        request.user = self.user
