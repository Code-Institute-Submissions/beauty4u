...
from .forms import HoursForm, aboutForm, addProductForm, staffForm, couponForm, addCategory, addBrand, updateProduct, addServiceForm, addServiceCategoryForm
...
def manage(request):
    ...
    next_7_bookings = Bookings.objects.filter(date__lt=next_week, date__gt=today, confirmed=False).count()
    this_week_bookings = Bookings.objects.filter(date__lt=next_week, date__gt=today).count() + todays_bookings
...
def order_detail(request, order_number2):
    ...
    A view that returns the details of an order, taking the order number as a parameter
...
def settings(request):
    ...
    A view that returns the settings template allowing the owner to change settings on their site
...
def staff(request):
    ...
    A view that returns all staff members and a form allowing the owner to add staff members 
...
def update_staff_avail(request):
    ...
    This view handles POST requests from the staff page and updates the availability of the staff member
...
def changeHours(request):
    ...
    """ 
    A view that will return the change hours template with a form allowing the user to update the opening hours 
...
def changeAbout(request):
    ...
    """ 
    A view that will return the change about us template and a form allowing the user to change the about us text displayed on the home page
...
def coupons(request):
    ...
    A view that will display all coupons and a form allowing the user to add a new coupon 
...
def update_coupon_minspend(request):
    ...
    """
    This view handles POST requests from postsettings.js 
    It allows the customer to update the minimum spend setting on a selected coupon
...
def manage_products(request):
    ...
    A view that returns all products to the template and allows users to filter products 
...
def add_a_product(request):
    ...
    A view that returns a form allowing users to add a product to the store 
...
def manage_specific_product(request, product_name):
    ...
    A view that returns a prefilled form with a prodyct allowing the user to alter details of that product
    ...
    return render(request, "management/dashboard_manage_specific_product.html", context)


def add_a_category(request):

    """ 
    A view the returns all categories and a form allowing the user to create a category
...
def add_a_brand(request):
    ...
    """ 
    A view the returns all brands and a form allowing the user to create a brand
    ...
    return render(request, "management/dashboard_add_brand.html", context) 
...
def delete_a_brand(request):
    ...
    """
    A view that thakes POST requests from postsettings.js and deletes the selected brand 
...
def delete_a_category(request):
    ...
    """
    A view that thakes POST requests from postsettings.js and deletes the selected category
...
def update_booking_status(request):
    ...
    Sends an email to the customer to say their booking has been confirmed with the toggler is clicked
...
def view_booking_details(request, booking_id):
    ...
    return render(request, 'management/dashboard_booking_details.html', context)
...
def send_marketing_email(request):
    ...
    A view that returns a template allowing the user to send an email to all users that are not superusers
    ...
    if request.method == "POST":
        ...
        # Open connection to gmail to send emails - use send_mass_email for efficiency to only open one connection
        try:
            #Create individual instances so that email list is not shared with all 
            message = [(email_subject, email_body, "marketing@beauty4u.ie", [email]) for email in list_of_emails]
...
def addService(request):
    ...
    """
    A view that returns all services and a form allowing the user to add a service
...
def addServiceCategory(request):
    ...
    """
    A view that returns all service categories and a form allowing the user to add a service category
